...
debuginfo_eip(uintptr_t addr, struct Eipdebuginfo *info)
{
	...
	if (addr >= ULIM) {
		...
		if (user_mem_check(curenv, usd, sizeof(struct UserStabData), 0)) return -1;
		...
		// LAB 8: Your code here.
		if (user_mem_check(curenv, stabs, (int)(stab_end - stabs), 0) ||
			user_mem_check(curenv, stabs, (int)(stabstr_end - stabstr), 0)) {
				return -1;
			}
	}
...
int stabname_equals(const char* stab_name, const char* fname) {
	// stab_name has a format of "fname:F(num, num)".

	int l = strlen(fname);
	return stab_name[l] == ':' && strncmp(stab_name, fname, l) == 0;
}
...
find_function(const char * const fname)
{
	...
	for (stab = stabs; stab <= stab_end; stab++) {
		...
		if (stabname_equals(stab_name, fname)) return stab->n_value;
	}
